// TODO change it so that buffers can be allocated dynacmically i.e not waste memory with maximum frame length
...
auto smbPitchShift(const float  pitchShift,
                   const long   numSampsToProcess,
                   const long   fftFrameSize,
                   const long   osamp,
                   const float  sampleRate,
                   const float* indata,
                   float*       outdata) -> void {

    // memory requirement at least 11 * MAX_FRAME_LENGTH e.g 11 * 8192 * 4 = 360448 bytes ( 352 KB )
    ...
    freqPerBin    = sampleRate / static_cast<double>(fftFrameSize);
    expct         = 2. * M_PI * static_cast<double>(stepSize) / static_cast<double>(fftFrameSize);
    ...
    for (i = 0; i < numSampsToProcess; i++) {
        ...
        if (gRover >= fftFrameSize) {
            ...
            for (k = 0; k < fftFrameSize; k++) {
                window                = -.5 * cos(2. * M_PI * static_cast<double>(k) / static_cast<double>(fftFrameSize)) + .5;
            ...
            for (k = 0; k <= fftFrameSize2; k++) {
                ...
                tmp -= static_cast<double>(k) * expct;
 ...
 * @param sign
 */
void smbFft(float* fftBuffer, const long fftFrameSize, const long sign)
/*
*/
{
    ...
    for (k = 0, le = 2; k < static_cast<long>(log(fftFrameSize) / log(2.) + .5); k++) {
        le <<= 1;
        le2 = le >> 1;
        ur  = 1.0;
        ui  = 0.0;
...
double smbAtan2(const double x, const double y) {
    double signx;
    if (x > 0.) signx = 1.;
    else
        signx = -1.;
